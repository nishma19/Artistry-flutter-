...
class OrderCheckoutPage extends StatefulWidget {
  ...
  OrderCheckoutPage({required this.orderId});
...
class _OrderCheckoutPageState extends State<OrderCheckoutPage> {
  ...
  Future<void> fetchOrderDetails() async {
    try {
      ...
      if (doc.exists) {
        ...
        showAlertDialog(context, "Error", "Order not found.");
        setState(() {
          isLoading = false;
        });
      }
    } catch (e) {
      showAlertDialog(context, "Error", "Failed to fetch order details.");
      setState(() {
        isLoading = false;
      });
    }
  ...
  void calculateTotalValues() {
    double taxRate = 0.18;
    subtotal = orderData?['productPrice'] ?? 0.0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Order Checkout', style: TextStyle(color: Colors.white)),
      ),
      body: isLoading
          ? Center(child: CircularProgressIndicator())
          : orderData == null
          ? Center(child: Text('Order details not available', style: TextStyle(color: Colors.white)))
          : Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              '${orderData!['productName']} - ${orderData!['productDescription']}',
            ...
            Row(
              ...
              children: [
                Text(
                  'Subtotal: \u{20B9} ${orderData!['productPrice'].toStringAsFixed(2)}',
